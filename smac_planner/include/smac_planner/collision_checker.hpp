...
public:
   ...
   */
  GridCollisionChecker(nav2_costmap_2d::Costmap2D* costmap) : FootprintCollisionChecker(costmap)
  {
  }
  ...
  bool inCollision(const float& x, const float& y, const float& theta, const bool& traverse_unknown)
  {
    ...
    // RCLCPP_INFO(_logger, "inCollision 1");
    if (!footprint_is_radius_)
    {
      // if footprint, then we check for the footprint's points
      footprint_cost_ = footprintCostAtPose(wx, wy, static_cast<double>(theta), unoriented_footprint_);
...
protected:
  ...
  rclcpp::Logger _logger{ rclcpp::get_logger("collision_checker") };
};
