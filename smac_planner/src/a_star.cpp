...
template <typename NodeT>
void AStarAlgorithm<NodeT>::initialize(const bool& allow_unknown, int& max_iterations,
...
template <>
void AStarAlgorithm<Node2D>::createGraph(const unsigned int& x_size, const unsigned int& y_size,
                                         const unsigned int& dim_3_size, nav2_costmap_2d::Costmap2D*& costmap)
{
  if (dim_3_size != 1)
  {
...
template <>
void AStarAlgorithm<NodeSE2>::createGraph(const unsigned int& x_size, const unsigned int& y_size,
                                          const unsigned int& dim_3_size, nav2_costmap_2d::Costmap2D*& costmap)
{
  _costmap = costmap;
...
template <>
typename AStarAlgorithm<NodeSE2>::NodePtr AStarAlgorithm<NodeSE2>::addToGraph(const unsigned int& index)
{
...
template <>
void AStarAlgorithm<NodeSE2>::setStart(const unsigned int& mx, const unsigned int& my, const unsigned int& dim_3)
{
...
template <>
void AStarAlgorithm<NodeSE2>::setGoal(const unsigned int& mx, const unsigned int& my, const unsigned int& dim_3)
{
  _goal = addToGraph(NodeSE2::getIndex(mx, my, dim_3, getSizeX(), getSizeDim3()));
  _goal_coordinates = NodeSE2::Coordinates(static_cast<float>(mx), static_cast<float>(my), static_cast<float>(dim_3));
  _goal->setPose(_goal_coordinates);

  NodeSE2::computeWavefrontHeuristic(_costmap, static_cast<unsigned int>(getStart()->pose.x),
...
bool AStarAlgorithm<NodeT>::createPath(CoordinateVector& path, int& iterations, const float& tolerance)
{
  ...
  while (iterations < getMaxIterations() && !_queue.empty())
  {
    ...
    neighbors.clear();
    NodeT::getNeighbors(current_node, neighborGetter, _collision_checker, _traverse_unknown, neighbors);
...
template <>
AStarAlgorithm<NodeSE2>::NodePtr AStarAlgorithm<NodeSE2>::getAnalyticPath(const NodePtr& node,
                                                                          const NodeGetter& node_getter)
{
  ompl::base::ScopedState<> from(node->motion_table.state_space), to(node->motion_table.state_space),
      s(node->motion_table.state_space);
  ...
  for (float i = 1; i < num_intervals; i++)
  {
    ...
    index = NodeSE2::getIndex(static_cast<unsigned int>(reals[0]), static_cast<unsigned int>(reals[1]),
...
template <typename NodeT>
typename AStarAlgorithm<NodeT>::NodePtr AStarAlgorithm<NodeT>::getAnalyticPath(const NodePtr& node,
                                                                               const NodeGetter& node_getter)
{
  return NodePtr(nullptr);
}
...
template <typename NodeT>
float AStarAlgorithm<NodeT>::getTraversalCost(NodePtr& current_node, NodePtr& new_node)
{
...
template <typename NodeT>
typename AStarAlgorithm<NodeT>::NodePtr AStarAlgorithm<NodeT>::tryAnalyticExpansion(const NodePtr& current_node,
                                                                                    ...
                                                                                    int& closest_distance)
{
  if (_motion_model == MotionModel::DUBIN || _motion_model == MotionModel::REEDS_SHEPP)
  {
    ...
    closest_distance =
        std::min(closest_distance,
                 static_cast<int>(NodeT::getHeuristicCost(node_coords, _goal_coordinates) / NodeT::neutral_cost));
